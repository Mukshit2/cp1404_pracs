...
def display_places(places):
    ...
    print(f"Name{' '*(max_name_len-4)}Country{'  '*(max_country_len-7)}Priority  Visited")
    print("-"*(max_name_len+max_country_len+20))
    sorted_places = sorted(places, key=lambda place: (place[3], -int(place[2])))
    for place in sorted_places:
        visited_mark = UNVISITED_MARK if place[3] == "n" else " "
        print(f"{place[0]:{max_name_len}} in {place[1]:{max_country_len}}"
              f"{place[2]:>8}{visited_mark}")
    if places_left == 0:
...
def main():
    places = load_places()
    print("Travel Tracker 1.0 - by Your Mukshit")
